...
# -- Generate and upload HTML page + archive pages ---------
subprocess.run(['python', '/home/ramblinray/mound/scripts/generate_page.py'])

page_scp = subprocess.run([
    'scp',
    '/home/ramblinray/mound/page/data_live.html',
    'ramblinray@192.168.1.33:/var/www/html/mound/data_live.html'
])
...
# -- Sync archive pages (index + month pages) --------------
archive_scp = subprocess.run([
    'scp', '-r',
    '/home/ramblinray/mound/page/archive',
    'ramblinray@192.168.1.33:/var/www/html/mound/'
])
if archive_scp.returncode == 0:
    print("Archive pages synced to ramblinray!")
else:
    with open(error_path, 'a') as f:
        f.write(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, ERROR: archive page SCP failed\n")
    print("Archive page SCP failed!")
